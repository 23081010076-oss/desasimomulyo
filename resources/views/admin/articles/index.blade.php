...
@section('content')
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-slate-900 dark:text-white">Artikel & Berita</h1>
            <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">Kelola publikasi dan berita kelurahan</p>
        </div>
        <a href="{{ route('admin.articles.create') }}" class="flex items-center gap-2 rounded-lg bg-cyan-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-cyan-700 dark:bg-cyan-500 dark:text-slate-950 dark:hover:bg-cyan-400">
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900/50 dark:shadow-xl dark:backdrop-blur-sm">
            ...
            <div class="flex items-center justify-between border-b border-slate-200 p-5 last:border-b-0 hover:bg-slate-50 transition dark:border-white/10 dark:hover:bg-white/5">
                <div class="flex items-center gap-4">
                    @if($article->featured_image)
                        <img src="{{ asset($article->featured_image) }}" alt="{{ $article->title }}" class="h-16 w-16 rounded-lg object-cover border border-slate-200 dark:border-white/10">
                    @else
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-slate-400">
                            ...
                            <span class="{{ $article->is_published ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400' }}">
                ...
                <div class="flex items-center gap-3 text-sm font-medium">
                    <a class="text-cyan-600 transition hover:text-cyan-700 dark:text-cyan-400 dark:hover:text-cyan-300" href="{{ route('admin.articles.edit', $article) }}">Edit</a>
        ...
        @empty
            <div class="p-8 text-center text-slate-500 dark:text-slate-400">Belum ada artikel yang ditambahkan.</div>
        @endforelse
    </div>
